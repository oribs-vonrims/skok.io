...
We defined postall address schema and than we are referencing it inside
...
create a tiny helper function which we will be using later on.
...
In the code above we are adding postal address with 3 properties - country, locality, and
region. There are many others properties, so make sure to check out the official
[`PostalAddress` page](https://schema.org/PostalAddress) and add them by your own needs
(this will also apply to the rest of the scehams).
...
of social media profiles, and `address` by referencing previously defined `Address` schema.
...
[schema.org](https://schema.org/Article) tells as that `publisher` property (which
is a required field for article schema) can be either organization or a person Google's
structured data validator tool will throw an error. And it's unfortunate.
...
`name`, and `address` we will add it as well.
