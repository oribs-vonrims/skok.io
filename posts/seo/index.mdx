...
2. The project I'm going to show will have a certain project structure,
   ...
   because it should be pretty easy to adopt the same principles for a project with
...
- `speakableSelector` this is a selector which will tell search engines
  and some other assistive technologies that a particular section on the
  page is "speakable".
...
Everything else should be pretty self-explanatory. Now you can go ahead and replace
all the values with your own information, but keep page's `type` field as is.
...
looking for. Create `hooks` directory in `./src` - `touch src/hooks/useSiteMetadata.js`
...
and template (`./src/templates/article.js`) level and than will be passed
...
the `Layout` component. `Layout` component will be a place where all the
...
You might be wondering why we can't use the same image? Well, because all social
networks and search engines have different requirments.
...
We are defining fragment name `ChildImageSharpFields` that will be
referenced in other queries. Specifing type name `ImageSharp` of an
object the fragment will be used on. Than we move aliases from the
initial example in the query body. And to make this fragment accessible
inside of the other GraphQL queries we use named export (There is no need
...
so we can finish pulling all the data we need in all the pages and
template.
...
On line `31-39` we define the query for the cover image and the data
coming from this query becomes accessible via `data` prop in `Home`
component. Let's grab the part we are interested in - `covers` and
spread it in `covers` prop of the `Layout` component.
...
the data passed to it from pages and template and fetches the remaining
...
We are wrapping `html` tag in `Helmet` component, that takes care of
rendering it's content inside of the `head` tag of HTML document.
...
- `og:type` the type of your object, which is represented in Open Graph. It can be
  music, video, website, profile, article, and serveral others. In our blog
...
it requires to Facebook developer's account (it's free, but you will spend a precious
minute or two filling out the form).
...
- `twitter.card` is a type of a card. We are using "Summary Large Image" which
  ...
  types, like "Player" or "Card", but we are not going to use them. (Required field).
...
Schema.org is a structured data markup, which helps search engines interpret
the content of a web pages. It was initially launched in 2011 by Google, Yahoo,
and Bing. Once added to a webpage it conveys the contextual meaning
to search engines by using hierarchical set of schemas.

![Schema.org Graph](https://upload.wikimedia.org/wikipedia/commons/8/8e/Schema.org_%285925660995%29.png)

HTML lacks the ability to provide a meaning of what a text string means. E.g.
a webpage can have a header of `<h1>Apple</h1>` and it can refer to a
fruit or a company. This is when schema.org comes into play and provides
semantical meaning to search engines.

There are several different formats in which you can represent schema.org, but
the most popular one is [JSON-LD](https://json-ld.org/). By far this is the most
intuitive for all developers, because it's almost indentical to a well know JSON
format. It's implementation doesn't dependant on the markup as opposed to other
schema.org formats which attach special attributes to HTML elements and are more
prone to errors.
...
order to be practical we are going to focus on the schemas, which are [officially
supported by Google](https://developers.google.com/search/docs/guides/search-gallery).
This way we are going to help search engines as well as benefit from rich snippets.
Rich snippets stand out from the other search results, look more appealing and have a
higher click thru rate.

Before jumping into coding I want to make you familiar with the concept of node
identifiers. To put it in simple words it's a unique `@id` property, which gets
attach to a schema and than can be referenced inside of another schema. This is
extremely useful for keeping your code DRY and readable. Take a look at this
example:
...
We defined postal address schema and than we are referencing it inside
of the person schema. Keep in mind that this is a simplied example and
real world schemas can get pretty bulky.

### Node identifier utility

Node identifiers can be pretty much anything, but the convention is to
use the domain name and hash `https://example.com/#your-schema`. Let's
create a tiny helper function which we will be used later on.
...
We are directly importing object from `site-metadata.js`, because
we can't use hooks outside of React componets. We will be able to call this function
like, so: `schemaId("address")` and it will return `https://example.com/#address`.
...
In the code above we are adding postal address with 3 properties - `country`, `locality`,
and `region`. There are many others properties, so make sure to check out the official
[`PostalAddress` page](https://schema.org/PostalAddress) and add them by your own needs
(this also applies to the rest of the scehams).

In order to inject JSON object in the `script` tag we are using a combination of
`dangerouslySetInnerHTML` and `JSON.stringify`.
...
For the person schema we are adding our `name`, blog `url`, `sameAs` - which is a list
of our social media profiles, and `address` by referencing previously defined `Address` schema.

### Organization Schema Component

Next one in our list is the `Organization` schema. You might think why would we need
it if we are adding markup for a personal blog. Well it's going to be required
later on when we will be adding `Article` schema. Even though
[schema.org](https://schema.org/Article) tells us that `publisher` property (which
is a required field for the article schema) can be either organization or a person in
practice Google's structured data validator tool will throw an error.
...
It's suficient to add `logo` and `name`, but because we already have `url`,
`name`, and `address` we will add them as well.

### Page Schema Component

Once we are done with smaller pieces we can start working on the bigger schemas.
We will create a reusable Page component and handle `WebPage`, `Blog`,
`ContactPage`, `AboutPage`, and `Article` all together, because they are very
simillar. Let's write down the part, which will be identical for all of the pages:
...
- `publisher` - content publisher. We are using node reference by passing the id of the
  organization schema (As you might remeber we specifically defined it to satisfy Google's
  structured data validator).
- `headline` - page headline.
- `inLanguage` - language of the content.
- `url` - URL of the page.
- `mainEntityOfPage` - indicates a page for which this thing is the main entity being
  described. In case of the blog the primary topic is clear, so we are going to use the same
  value as `url`. (For more complecated websites this might not be the case).
- `image` - list of images with 1x1, 4x3, and 16x9 aspect ratios.

There are also properties, which we will be added conditionally depending on the page type.
For the `Article` page it will be `datePublished`, and `dateModified`. And `Blog` page can't
have `speakable` property.

`speakable` indicates sections of a Web page that are particularly
'speakable' in the sense of being highlighted as being especially appropriate for
text-to-speech conversion. [Google](https://developers.google.com/search/docs/data-types/speakable)
recently started using speakable structured data to answer topical news queries on smart
speaker devices.
...
In order to conditionally add properties to the object we are leveraging
`Object.assign(target, ...sources)`. All static properties is our `target`
and all optional properties is our `source`. If you want to dig into this
syntax a little further I recommend to check out this
...
`dateModified` needs an extra bit of logic. In order not define
`dateModified` in the frontmatter for every new blog post we check if
it exists and than we apply it, otherwise we are reusing `datePublished`
value (we are doing it because `dateModified` is the reqiured property
on the `Article` schema).

### Breadcrumbs Schema Component

`BreadcrumbList` is independant and doesn't appear to be a part of any
...
`0` as all of you devs might thought at first 😂). The first item is the
root of the website.
...
On line 15-40 we construct an array of links depending on the page.
...
Crafting `Head` and `SchemaOrg` components will help you improve your organic
SEO as well as social media sharing, but do remember that this is not a
silver bullet. Think about it as complementary thing for a great content you are
