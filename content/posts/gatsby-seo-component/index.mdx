...
In part #3, we will create `SEO` component. It will render Twitter, Open Graph, and Schema.org meta tags.
...
`SEO` component receives data from every page and acts as a wrapper for other smaller components. Based on the
received data it constructs `imageUrls`, `activePages` objects, and `url` string that are passed down the
...
After appling [`ImageUrlFields` fragment](/blog/gatsby-graphql-fragments/#imageurlfields-graphql-fragment) on a File node
we get back a deeply nested object with multiple relative paths. The purpose of `getImageUrls` function is to generate a
URL for every path and pack it in a consice object.
...
All page types have different markup templates. E.g. `/contact` page corresponds to [ContactPage](https://schema.org/ContactPage)
schema and `/blog/pug/` page corresponds to [Article](https://schema.org/Article) schema, but there
is no mechanism within `SEO` component to distinguish between these two pages thus no way to know which template to use.

`getActivePages` helper takes care of identifing page type of the received data. It does that by checking for
`pageId` prop against a list of all pages defined in [`site-metadata.js`](/blog/gatsby-node-and-config-files/#site-metadata).
Later on we will rely on it's output to map pages with their corresponding templates.
...
`Twitter` component contains twitter specific meta tags. When the URL on your blog we be shared via
twitter it will be expanded in attractive summary card with a large image.
...
The Open Graph protocol is supported by a wide range of social media platforms that includes Facebook
(it also created this protocol in 2010), LinkedIn, Pinterest, Twitter (when Twitter can't find twitter
...
- `og:type` the type of object that is represented in Open Graph (can be music, video, website, profile, article,
  and others). Its going to be article if renderred inside of the [`Article` page component](/blog/gatsby-graphql-fragments/#article-page-component),
...
launched in 2011 by Google, Yahoo, and Bing. Once added to a webpage it conveys the contextual meaning to search engines
by using a hierarchical set of schemas.
...
[Schema.org vocabulary](https://schema.org/docs/full.html) is vast and in order to be practical we are going to focus
only on the schemas, that are officially supported by Google. This way we are going to help search engines as well as
...
Before jumping straight to the code I would like to explain what node identifiers are. Its a unique `@id` property,
that gets attached to a schema and then can be referenced inside of another schema. This is extremely useful for
...
duplicating the whole `PostalAddress` object inside of the `Person` and `Organization` schemas.

Node identifier value can be pretty much anything, but the common practice is to use the domain name and
...
The rest of the properties we will be added conditionally depending on the page type. Article schema requires
...
`BreadcrumbList` schema doesnt appear to be a part of any other schema. It consists of a chain of linked web pages.
The position property is used to order the items and starts with `1`. Home page will always be the first item, that's
the reason why we define `breadcrumbList` with `siteUrl` right away. The second level is a blog, contact, or about page.
With our [URL structure](/blog/gatsby-graphql-fragments/#page-components-and-url-structure-overview) third level will always
be an article and it will always go after the blog page. That's why we push both of these values to the `breadcrumbList`
array at once.

Once all the schemas are constructed we are creating a node array using `@graph` and fill it in with schemas. The only
...
"Scrape Again" button fetch the latest version of the page. Most likely after clicking debug button you will see a
...
For schema.org markup validation I recommend to use [Google Structured Data](https://search.google.com/structured-data/testing-tool)
and [Google Structured Data](https://search.google.com/test/rich-results) in conjunction. In my experience there cases when
Google Structured Data will not find certain issues with your markup while Google Structured Data will and vice versa.

There is also an alternative tool to validate all social media cards at once [opengraph.xyz](https://www.opengraph.xyz/), but
it is not as comprehensive as the other tools.
