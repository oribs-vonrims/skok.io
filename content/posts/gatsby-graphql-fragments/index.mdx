...
In part #2, we will create five page components and will learn about GraphQL fragments and aliases.
We will also create `Layout`, `Nav` components and MDX blog posts with frontmatter.
...
that will be rendered by [`SEO` component](/blog/gatsby-seo-component/) (will be
covered in the next part of the series). Every blog post has its own folder with
`index.mdx` and a cover image.

## Home Page Component

All page components are stored in `src/pages` directory.
...
We import `graphql` from `gatsby` and then we `export` a new constant called `query` (its name can be anything).
`graphql` is a tagged template function and isnt executed the same way as a typical JavaScript code. During the
build step Gatsby converts the source code to an abstract syntax tree (AST) and removes `graphql`-tagged templates
from the original source code. For these reasons we can't use expression interpolation, but we can still pass
variables to the queries via `context` argument of `createPages` action (This is exactly what we did earlier
with [`image` absolute file path in `gatsby-node.js`](/blog/gatsby-node-and-config-files/#gatsby-nodejs) and now
it's available as `$image` inside of the query). Keep in mind that you can only have one GraphQL query per file.
...
syntax looks like JavaScript spread operator and has a similar purpose: to assign the keys and values of one object
to another object.

The result of the query is automatically inserted in `Home` component as the `data` prop. The rest of the page
data is retrieved from [`site-metadata.js`](/blog/gatsby-node-and-config-files/#site-metadata) via
[`useSiteMetadata` hook](/blog/gatsby-node-and-config-files/#usesitemetadata-hook).
...
- [Google](https://developers.google.com/search/docs/data-types/article) recommends images to be at least 696 pixels wide
  and suggests to provide images with 16x9, 4x3, and 1x1 aspect ratios.

We are going to use 1600 pixels as our width baseline.
...
To create a fragment, define it in a query and export it as a named `export`, that makes it globally available
(there is no need to import fragments before using them) regardless of its location in the project. All fragments
...
`Contact` page is almost identical to the [Home page component](/blog/gatsby-graphql-fragments/#home-page-component). The
...
`About` page is almost identical to the [`Home` page component](/blog/gatsby-graphql-fragments/#home-page-component),
but this time we we fetch `siteMetadata.pages.about` instead of `siteMetadata.pages.home`.
...
In the `Blog` page component we first need to get all MDX nodes (our articles) and
[sort](https://www.gatsbyjs.com/docs/graphql-reference/#sort) them in the descending order by the
`frontmatter___published` field (its defined in the article's frontmatter). After this we query for
`slug` node and [`FrontmatterFields` (GraphQL fragment) fields](/blog/gatsby-graphql-fragments/#frontmatterfields-graphql-fragment)
of the MDX nodes and loop over them to generate a list of all articles.
...
`FrontmatterFields` is the reusable piece of code that contains all frontmatter fields of an MDX articles. Here is the
example of the article's frontmatter:
...
`published` and `modified` fields have a type of `Date`, thus we can apply `formatString` function.
You can use [Moment.js](https://momentjs.com/docs/#/displaying/format/) syntax to format date strings.

`image` is a file path that was converted to a GraphQL `File` node and because its an image we can access
`childImageSharp` field. To configure image sizes, loading effect and file formats we pass arguments inside
of `gatsbyImageData` resolver. By setting the `width` we are limiting the maximum image size. Gatsby image
components are lazy-loaded by default and to ensure that the layout does not jump around, a placeholder is
displayed before the image loads. `BLURRED` option generates a low-resolution version of the source image
and displays it as a blurred background. [Gatsby Image plugin](https://www.gatsbyjs.com/docs/reference/built-in-components/gatsby-plugin-image/#image-options)
supports four output formats: JPEG, PNG, WebP and AVIF. `AUTO` means the plugin will generate images in the
same format as the source image. Then we also specify `WebP` and a new `AVIF` format (Currently `AVIF` has a
...
Inside of the `graphql` string we query MDX data by `id` (`id` is accessible inside of the query because we passed it in
the [`context`](/blog/gatsby-node-and-config-files/#createpages-api)). Then we spread `FrontmatterFields` fragment in
`frontmatter` node to get every frontmatter field. To get SEO images we spread `ImageUrlFields` fragment in the
`childImageSharp` node. The rest of the data is queried with [`useSiteMetadata` hook](/blog/gatsby-node-and-config-files/#usesitemetadata-hook).

In `Article` component we pass `body` (all MDX file content without frontmatter data) to the `MDXRenderer`. It's a
...
`Layout` component in Gatsby is used to share common styles and components between the pages. We use it to include
...
Not every page will require `SEO` component. If you ever decide to add a 404 page make sure that `SEO` component is not
rendered there. That's why we are using `pageId` prop from [`site-metadata.js`](/blog/gatsby-node-and-config-files/#site-metadata)
that is defined for every page that contains `SEO` component.

`data-speakable="true"` is appended to the `main` tag to let assistive technologies know that the content inside of it is
good for the Text-to-Speech interpretation.

## Nav component

Inside of the `Nav` component we use [`useSiteMetadata` hook](/blog/gatsby-node-and-config-files/#usesitemetadata-hook) to
...
[frontmatter](/blog/gatsby-graphql-fragments/#frontmatter-code-block) to the top of each file.
