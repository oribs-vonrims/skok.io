...
(its name can be anything). <Underline>`graphql` is a tagged template function and isn't executed
the same way as a typical JavaScript code</Underline>. During the build step, Gatsby converts the source code to an
...
can still pass variables to the queries via `context` argument of `createPages` action. This is exactly what we
did earlier with [`image` absolute file path](/blog/gatsby-node-and-config-files/#gatsby-nodejs) in `gatsby-node.js`.
That's the reason why it's available as `$image` inside of the query. <Underline>Keep in mind that you can only
...
Facebook, Twitter, Google all have different requirements for the size and aspect ratio for their cover images:
...
`frontmatter___published` field (it's defined in the article's frontmatter). After this, we query for
...
`FrontmatterFields` is the reusable piece of code that contains all frontmatter fields of an MDX article. Here is the
...
`published` and `modified` fields have a type of `Date`. Thus we can apply the `formatString` function.
...
before the image loads. `BLURRED` option generates a low-resolution version of the source image and displays it as
a blurred background. [Gatsby Image plugin](https://www.gatsbyjs.com/docs/reference/built-in-components/gatsby-plugin-image/#image-options)
...
`Article` component receives the MDX file and renders it.
...
In the `Article` component, we pass `body` (all MDX file content without frontmatter data) to the `MDXRenderer`. It's a
the component that takes compiled MDX content and renders it.
...
Not every page will require an `SEO` component. If you ever decide to add a 404 page, ensure
that the `SEO` component is not rendered there. That's why we are using `pageId` prop from
[`site-metadata.js`](/blog/gatsby-node-and-config-files/#site-metadata) that is defined for
every page that contains `SEO` component.

`data-speakable="true"` is appended to the `main` tag to let assistive technologies know
that the content inside of it is suitable for the Text-to-Speech interpretation.
