...
`sitemap.xml` is a way to provide more information for search engines about pages on your website. A sitemap is a file that
...
By default, `gatsby-plugin-sitemap` expects site URL from `siteMetadata.siteUrl`, so we don't need to
do anything else. <Underline>It's important to note that this plugin generates sitemap files only in
`production` mode. To validate generated `sitemap.xml` file you will need to build and serve the
site.</Underline>
...
The sitemap index page solves this issue by letting you break down your sitemap into smaller
pieces. For those of you who are interested to know what will be the max amount of URLs
...
As it turns out, `changefreq` and `priority` are not used by Google anymore. We can safely remove this bloat from
`sitemap.xml` and add what's important - last modified date `lastmod`.
...
and article URLs. Then we fetch the last modified date for each article and create a sitemap entry for all the articles. When
the article doesn't have the last modified date, we fall back to the published date.
...
To instruct search engines how to crawl pages on your website, there is `robots.txt` file. It indicates whether a particular user
agent can or cannot crawl parts of a website. These crawling instructions are specified by `disallow` and `allow` directives
